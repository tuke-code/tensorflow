...
absl::Status ShapeVerifier::CheckAsyncUpdateOperands(
  ...
  if (!CheckShapePrefix(shape0.tuple_shapes(0), shape1.tuple_shapes(0))) {
    return Internal(
  ...
  if (!CheckShapePrefix(shape0.tuple_shapes(1), shape1.tuple_shapes(1))) {
    return Internal(
...
absl::Status ShapeVerifier::CheckAsyncDoneOperands(
  ...
  // in CheckAsyncOpComputationShapes, which ensures that the shape of
...
absl::Status ShapeVerifier::CheckAsyncOpComputationShapes(
  ...
  if (async_op->opcode() == HloOpcode::kAsyncStart ||
      async_op->opcode() == HloOpcode::kAsyncUpdate) {
    if (!CheckShapePrefix(async_shape->tuple_shapes(0), param_shape)) {
      return Internal(
    ...
    if (!CheckShapePrefix(async_shape->tuple_shapes(1),
